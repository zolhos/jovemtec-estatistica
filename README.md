...
- **Classificador Gamificado de Variáveis:** Teste interativo para fixação dos 4 tipos de variáveis (Qualitativa Nominal, Ordinal, Quantitativa Discreta e Contínua) com base em dados do Censo e do UNICEF.
- **Simulador Interativo Outlier ("Efeito Neymar"):** Demonstração tátil de como a Média é disparada por *outliers* enquanto a Mediana permanece estável em R$ 1.412,00.
- **Construção Completa da Dispersão (Bloco 3):** Apresentação matemática em 3 etapas: **Desvio Simples** ($d_i = x_i - \bar{x}$, cuja soma é zero), **Variância** ($\sigma^2$, diferenças quadráticas) e **Desvio Padrão** ($\sigma = \sqrt{\sigma^2}$, unidade original).
- **Simulador de Alocação de Verbas e Equidade ($\sigma$):** Ajuste de orçamentos escolares para medir a variação do Desvio Padrão ($\sigma$) em tempo real.
- **Oficina de Resolução COTUCA:** Quiz com 5 questões históricas do COTUCA e gabarito sociomatemático comentado.
